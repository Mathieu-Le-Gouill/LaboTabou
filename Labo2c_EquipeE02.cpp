...
//NB:uneSol ne doit pas etre modifiee (const)
TSolution AppliquerVoisinage(const TSolution uneSol, TProblem unProb, TAlgo& unAlgo, vector<int>& bloc);


bool SolEstAdmissible(const vector<vector<int>>& listeTaboux, const TSolution& sol, long critere_aspiration, const vector<int>& bloc);

//DESCRIPTION: Deplacement de 1-2-3 villes selectionnees aleatoirement.												
void OrOPT(TSolution& Voisin, TProblem unProb, TAlgo& unAlgo, vector<int>& bloc);
...
bool SolEstAdmissible(const vector<vector<int>>& listeTaboux, const TSolution& sol, long critere_aspiration, const vector<int>& bloc)
{
	...
	if (std::find(listeTaboux.begin(), listeTaboux.end(), bloc) == listeTaboux.end() || sol.FctObj < critere_aspiration)
	{
...
TSolution AppliquerVoisinage(const TSolution uneSol, TProblem unProb, TAlgo& unAlgo, vector<int>& bloc)
{
	...
	OrOPT(Copie, unProb, unAlgo, bloc);
